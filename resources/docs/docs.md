...
The `home-routes` handler in the `oluet-api.routes.home` namespace
...
The `home-page` function will in turn call the `oluet-api.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/oluet_api/core.cljs` file.



<p class="title is-5">Organizing the routes</p>

The routes are aggregated and wrapped with middleware in the `oluet-api.handler` namespace:
...
Request middleware functions are located under the `oluet-api.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `oluet-api.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `oluet-api.db.core` in some other namespace.
